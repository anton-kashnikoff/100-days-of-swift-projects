...
struct CardSize {
    ...
    func getCardSize(collectionView: UICollectionView) -> CGSize {
        ...
        let (widthCardNumber, heightCardNumber) = getWidthHeightCardNumber(
        ...
        if let layout {
            layout.sectionInset = UIEdgeInsets(
                top: layout.sectionInset.top + heightRemainder,
                left: layout.sectionInset.left + widthRemainder,
                bottom: layout.sectionInset.bottom + heightRemainder,
                right: layout.sectionInset.right + widthRemainder
            )
        }
    ...
    func getWidthHeightCardNumber(
        availableWidth: CGFloat,
        availableHeight: CGFloat
    ) -> (
        widthCardNumber: CGFloat,
        heightCardNumber: CGFloat
    ) {
    ...
    func getSideWithClosestRatio(referenceRatio: CGFloat, imageSide1: CGFloat, imageSide2: CGFloat) -> Int {
        ...
        let gridSide1IsClosest = if gridSide1Ratio >= 1 && gridSide2Ratio >= 1 {
            // pick the closest ratio
            abs(gridSide1Ratio - referenceRatio) <= abs(gridSide2Ratio - referenceRatio)
        } else if gridSide1Ratio < 1 && gridSide2Ratio < 1{
            // none oriented correctly: pick the closest to 1
            gridSide1Ratio >= gridSide2Ratio
        } else {
